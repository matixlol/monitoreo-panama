...
const DATE_TOOLTIP_CSS = `
  ...
  .mf-date-tooltip::after{
    content:none;
  }

  .mf-date-tooltip[data-state='closed']{
    opacity:0;
    pointer-events:none;
  }

  .mf-date-tooltip-arrow{
    fill:#fffef3;
  }

  .mf-date-tooltip-arrow polygon{
    fill:#fffef3;
    stroke:#d0d7de;
    stroke-width:1px;
  }
...
const PARITY_MAP_COLORS = {
  male: '#3089b8',
  midpoint: d3.interpolateRgb('#3089b8', '#d96b8f')(0.5),
  female: '#d96b8f',
};
...
function appendChartSlot(body, node) {
  const slot = document.createElement('div');
  slot.className = 'wc-chart-slot';
  slot.append(node || Object.assign(document.createElement('div'), { className: 'empty', textContent: 'Sin datos.' }));
  body.append(slot);
}
  ...
  class PanamaChartElement extends HTMLElement {
    ...
    async render() {
      ...
      const renderResolved = (store) => {
        ...
        const isParityLayout = node?.classList?.contains('mf-parity-layout');
        ...
        if (isParityLayout) body.classList.add('wc-body--parity');
        if (controls.length === 1) {
          body.append(
            createTabsControl(
              controls[0],
              (value) => this.setAttribute(controls[0].attr, value),
              isParityLayout ? 'wc-tabs-wrap wc-tabs-wrap--parity' : 'wc-tabs-wrap',
            ),
          );
