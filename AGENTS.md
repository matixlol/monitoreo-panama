...
- Observable integration contract: when using `@modal-scripts/fichas-modales.js` in the notebook, the notebook must set `window.documentosIngresos` and `window.documentosEgresos`. Treat those globals as required API surface; don't remove them unless the modal script integration is updated too.
