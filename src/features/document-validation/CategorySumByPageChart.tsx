...
export function CategorySumByPageChart({ ingressRows, egressRows, onPageClick, currentPage }: Props) {
  const containerRef = useRef<HTMLDivElement>(null);
  const svgRef = useRef<SVGSVGElement>(null);

  const pageData = useMemo(() => {
    const pageAmounts = new Map<number, { ingress: number; egress: number }>();
    ...
    for (const row of egressRows) {
      ...
      existing.egress += catSum;
      pageAmounts.set(page, existing);
    }

    const data: PageData[] = [];
    for (const [page, amounts] of pageAmounts) {
  ...
  useEffect(() => {
    ...
    const y = d3
      .scaleLinear()
      .domain([0, d3.max(pageData, (d) => d.amount) || 0])
      .nice()
      .range([height, 0]);
  ...
  if (pageData.length === 0) {
    return (
      <div className="h-full flex items-center justify-center text-slate-400 text-sm">
        No hay datos con montos para graficar
      </div>
    );
  }
