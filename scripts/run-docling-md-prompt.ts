...
const doclingHint = `\n\n<docling_alignment_hint>\nDocling produced the following markdown table for this page.\nUse it ONLY to understand column alignment and empty cells.\nDo NOT copy its text as the source of truth if the PDF contradicts it.\nDocling markdown excerpt:\n${trimmedMd}\n</docling_alignment_hint>`;
...
console.log(`[${MODEL.id}] Docling-markdown-assisted extraction, runs=${runs}`);
...
console.log(`Docling-markdown batch: correct=${correct}, incorrect=${incorrect}, unknown=${unknown}`);
...
if (correct === runs && incorrect === 0) {
  ...
  console.log(`Docling-markdown batch 2: correct=${correct2}, incorrect=${incorrect2}, unknown=${unknown2}`);
}
