...
const doclingSummary = json.length > 7000 ? json.slice(0, 7000) + '\n...[truncated]' : json;
...
console.log(`[${MODEL.id}] Docling-JSON-assisted extraction, runs=${runs}`);
...
console.log(`Docling-JSON batch: correct=${correct}, incorrect=${incorrect}, unknown=${unknown}`);
...
if (correct === runs && incorrect === 0) {
  ...
  console.log(`Docling-JSON batch 2: correct=${correct2}, incorrect=${incorrect2}, unknown=${unknown2}`);
}
